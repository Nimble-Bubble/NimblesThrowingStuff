...
{
    public class ChlorophyteGreatlanceProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!projOwner.frozen)
            {
                Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 128, Projectile.velocity.X / 3, Projectile.velocity.Y / 3, 100, default(Color), 1f);
            ...
            if (Main.mouseRight && !hasShelled)
            {
                if (NimblesThrowingStuff.MIGuardKey.Current)
                {
                    ...
                    for (int f = 0; f < 5; f++)
                    {
                        int fireIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 128, 0f, 0f, 100, default(Color), 2f);
                ...
                else
                {
                    if (projOwner.GetModPlayer<NimblesPlayer>().currentShells > 0)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity, ProjectileID.SporeCloud, Projectile.damage / 4 * 5, 1.5f, Projectile.owner);
                           SoundEngine.PlaySound(SoundID.Item64);
                        for (int f = 0; f < 20; f++)
                        {
                            int fireIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 107, 0f, 0f, 100, default(Color), 2f);
                            Main.dust[fireIndex].velocity *= 4f;
                        }
                        hasShelled = true;
                    ...
                    for (int s = 0; s < 5; s++)
                    {
                        int smokeIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 128, 0f, 0f, 100, default(Color), 1.5f);
