	...
	public class NanoMissileProj: ModProjectile
    {
        ...
        public override void Kill(int timeLeft) 
        {
                        Projectile.NewProjectile(projectile.position.X, projectile.position.Y, 0, 0,
                            mod.ProjectileType("NanoMissileExplosion"), projectile.damage, 10f, projectile.owner, 0.0f, 0.0f);
            projectile.position = projectile.Center;
            projectile.width = 250;
            projectile.height = 250;
             projectile.Center = projectile.position;
		...
		{
			Texture2D texture = mod.GetTexture("Projectiles/Throwing/NanoMissileProj_Glow");
			spriteBatch.Draw
			(
				texture,
				projectile.position,
